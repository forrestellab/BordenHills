...
setwd('/Volumes/GoogleDrive/.shortcut-targets-by-id/1r1oKDNHCr46TTMS-33jRDHiZMbC6tDsI/Lodi_Heatwave/Data/Lodi_2021/BordenStations_2021/Stations_2021') #Directory where the raw data is in Google Drive
Dir <- getwd()

# Setting a Directory for the outcome tables
Dir_Outcome <- c("/Volumes/GoogleDrive/.shortcut-targets-by-id/1r1oKDNHCr46TTMS-33jRDHiZMbC6tDsI/Lodi_Heatwave/Data/Lodi_2021/BordenStations_2021/Outputs_2021/D18/")
...
for (i in 1:length(Stations_List)){
  ...
  for (j in 1:length(Folder_Station_Converted)){
    ...
    #Table_Thermocouples[[j]] <- Table_Thermocouples[[j]] %>% mutate(across(everything(), as.character))
  ...
  df_Table_Met <- do.call(bind_rows, Table_Met)
    if(is.null(Table_Thermocouples) == FALSE){
  ...
  df_Table_IRT <- df_Table_IRT %>% arrange(TIMESTAMP)
  ...
  # Deleting duplicated rows (sometimes the datalogger overwrites rows in between extraction dates)
  df_Table_IRT <- df_Table_IRT %>% distinct(TIMESTAMP, .keep_all = TRUE)
  df_Table_Met <- df_Table_Met %>% distinct(TIMESTAMP, .keep_all = TRUE)
  if(is.null(Table_Thermocouples) == FALSE){
  df_Table_Thermocouples <- df_Table_Thermocouples %>% distinct(TIMESTAMP, .keep_all = TRUE)}
  if(is.null(Table_BB) == FALSE){ #Not all the stations have an Anemometer, only doing this for stations that have one
  df_Table_BB <- df_Table_BB %>% distinct(TIMESTAMP, .keep_all = TRUE)}
